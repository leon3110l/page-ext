const metaData_Module = (function () {
    ...
    function fillElements(formElements) {
        const content = formEements[0].querySelector('input').value;

        /* changes the select*/
        formEleents[1].querySelectorAll('select option')[1].selected = true;

        /* loads rubriek*/
        const rubriek = fomElements[2].querySelector('select');
        const id = formElements[1].querySelector('select').value;
        sendAjax2(id, rubriek)

        /* changes the meta titel*/
        formElements[3].querySelector('input').value = cotent;

        /* changes the meta titel*/
        formElements[4].querySelector('textarea').value = "alles over ";
        formElements[4].querySelector('textarea').value += contnt;
        ...
        xhr.onreadystatechange = function() {//Call a function when the state changes.
            if(xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
                ...
                for (var i = 0; i < response.length; i++) {
                    ...
                    opton.setAttribute('value', response[i]);
                    option.text = response[i];
                    rubriek.append(opton)
                }
            }
        }
        xr.send('id=' + id_l)
